...
// ═══════════════════════════════════════════════════════════════════════════════════
// NexusAI RAG Research Engine — v4.1 Architecture
// ═══════════════════════════════════════════════════════════════════════════════════
// 
// FEATURES:
// • Workspace: Upload PDFs, ask questions using RAG (Retrieval-Augmented Generation)
...
// • Web Search History: Click to view old searches and their results
//
// v4.1 CHANGES (session isolation):
// 1. Every API request now sends X-Session-ID header (anonymous browser isolation)
// 2. Session ID persisted in localStorage via src/lib/session.js
// 3. Endpoints renamed to match backend: /sessions → /chats, session_id → chat_id
// 4. Backend filters all data (files, chats, history) by this session ID
//
...
// Every request carries X-Session-ID so the backend can isolate this browser's
// documents, chats, and history from any other browser/device.
const req = (url, opts = {}) =>
  fetch(`${API}${url}`, {
    ...opts,
    headers: {
...
const api = {
  ...
  upload: (files) => {
    ...
    return fetch(`${API}/upload`, {
      method: "POST",
      headers: { "X-Session-ID": getSessionId() },   // no Content-Type — browser sets multipart boundary
      body: fd,
    }).then(r => r.ok ? r.json() : r.text().then(t => { throw new Error(t) }))
  },
  ...
  query:           (body)  => req("/query",      { method:"POST", body:JSON.stringify(body) }),
  webSearch:       (query) => req("/websearch",   { method:"POST", body:JSON.stringify({query}) }),

  // Chats (backend route is /chats, field is chat_id — matches main.py)
  sessions:        ()      => req("/chats"),
  sessionMessages: (id)    => req(`/chats/${id}/messages`),
  deleteSession:   (id)    => req(`/chats/${id}`, { method:"DELETE" }),
...
function Sidebar({ active, setActive, fileCount, sessions, webSearchHistory, currentSessionId, onSelectSession, onNewChat, onDeleteSession, onDeleteWebSearch, onSelectWebSearch, onNewSearch }) {
  ...
  return (
    <aside style={{ width:240, background:C.white, borderRight:`1.5px solid ${C.border}`, display:"flex", flexDirection:"column", flexShrink:0, height:"100%", overflow:"hidden" }}>
      ...
      {/*FOOTER */}
      <div style={{ padding:"16px 22px", borderTop:`1px solid ${C.border}`, fontSize:12, color:C.muted, lineHeight:1.7 }}>
        <div style={{ fontWeight:600, color:C.text, fontSize:13, marginBottom:3 }}>NexusAI RAG Engine</div>
        FastAPI · LangChain · FAISS
        <div style={{ display:"flex", alignItems:"center", gap:6, marginTop:7 }}>
          <div style={{ width:8, height:8, borderRadius:"50%", background:C.green, animation:"pulse 2s infinite" }} />
          <span>Groq · Llama 3.3-70b</span>
...
function WorkspacePage({ stats, setStats, sessionId, setSessionId, sessions, setSessions }) {
  ...
  const doQuery = async () => {
    ...
    const q = query; setQuery("")
    try {
      // chat_id matches backend's QueryRequest field name (was session_id before)
      const data = await api.query({ query:q, use_web:false, chat_id:sessionId })
      if (!sessionId) {
        setSessionId(data.chat_id)
        api.sessions().then(d => setSessions(d.chats||[])).catch(()=>{})
      }
      setMessages(prev => prev.map((m,i) => i===prev.length-1 ? { ...m, answer:data.answer, sources:data.sources||[], loading:false } : m))
      setStats(prev => ({ ...prev, queries:prev.queries+1 }))
    } catch(e) {
...
// Manages: active page, file list, chat sessions, web search history, selected search
// Key Features: localStorage persistence for web search, responsive layout, session isolation
...
export default function App() {
  ...
  // MOUNT-TIME INITIALIZATION
  // 1. Load indexed files and stats (scoped to this browser's session, via X-Session-ID)
  // 2. Load past chat sessions (scoped to this browser's session)
  // 3. Load persisted web search history from localStorage (survives page reload!)
  useEffect(() => {
    api.files().then(d => {
      if (d.files?.length) {
